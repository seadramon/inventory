	...
	<div class="col-12">
		<div class="card card-xl-stretch mb-5 mb-xl-8">
			<div class="card-header">
				<h3 class="card-title">IT Software</h3>
				<div class="card-toolbar">
					<a href="{{route('master.it-software.create')}}" class="btn btn-light-primary" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">Tambah Data</a>
				</div>
			</div>
			<div class="card-body">
...
<script type="text/javascript">
	...
	var KTDatatablesServerSide = function () {
	    ...
	    var initDatatable = function () {
	        dt = $("#tabel_tipe_pc").DataTable({
	            ...
	            order: [[0, 'asc']],
	            stateSave: true,
	            ajax: "{{ route('master.it-software.data') }}",
	            columns: [
	                {data: 'nama', name: 'nama', defaultContent: '-'},
	                {data: 'jenis_lisensi', name: 'jenis_lisensi', defaultContent: '-'},
	...
	$('body').on('click', '.delete', function () {
		if (confirm("Delete Record?") == true) {
			...
			$.ajax({
				type:"post",
				url: "{{ url('master/it-software/destroy') }}",
				data: {id : id, _token: "{{ csrf_token() }}"},
				success: function(res){
					if (res.result == 'success') {
						flasher.success("Data telah berhasil dihapus!");

						$('#tabel_tipe_pc').DataTable().ajax.url("{{ route('master.it-software.data') }}").load();
					}
				}
			});
		}
