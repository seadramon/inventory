		...
		<div class="card card-xl-stretch mb-5 mb-xl-8">
			<div class="card-header">
				...
				<div class="card-toolbar">
					<a href="{{route('master.ruangan.create')}}" class="btn btn-light-primary" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">Tambah Data</a>
				</div>
			</div>
			<div class="card-body">
...
<script type="text/javascript">
	...
	$('body').on('click', '.delete', function () {
		if (confirm("Delete Record?") == true) {
			...
			$.ajax({
				type:"post",
				url: "{{ url('master/ruangan/destroy') }}",
				data: {id : id, _token: "{{ csrf_token() }}"},
				success: function(res){
					if (res.result == 'success') {
						flasher.success("Data telah berhasil dihapus!");

						$('#tabel_tipe_pc').DataTable().ajax.url("{{ route('master.ruangan.data') }}").load();
					}
				}
			});
		}
